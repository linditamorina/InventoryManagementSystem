...
//   // 1. Hook-et e React Query me emra unikë për të shmangur konfliktet
...
//     // Kriojmë objektin që plotëson kërkesat e InsertProduct (Screenshot 6/8)
//     const productData = {
//       name: formData.name,
//       sku: formData.sku,
...
//       // Fushat default për të kënaqur TypeScript
//       description: "", 
...
//     if (editingProduct) {
//       // Përdorim .mutate() siç kërkohet nga React Query
...
//     if (confirm("A jeni të sigurt që dëshironi ta fshini këtë produkt?")) {
...
//                 <tr>
//                   <td colSpan={5} className="p-32 text-center text-slate-400 uppercase text-xs font-black italic">Asnjë produkt u gjet</td>
//                 </tr>
//               ) : (
//                 filteredProducts.map((p: any) => (
//                   <tr key={p.id} className="hover:bg-slate-50/80 transition-all group italic">
//                     <td className="p-8">
...
//                        <button onClick={() => handleDelete(p.id)} className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm"><Trash2 size={16}/></button>
//                     </td>
//                   </tr>
//                 ))
//               )}
...
//                   {editingProduct ? 'Edito Produktin' : 'Shto Produkt'}
//                 </h2>
...
//                   <input required type="text" value={formData.name} onChange={(e)=>setFormData({...formData, name: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600 font-bold transition-all" />
//                 </div>
//                 <div className="space-y-2">
//                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">SKU</label>
//                   <input required type="text" value={formData.sku} onChange={(e)=>setFormData({...formData, sku: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600 font-bold transition-all" />
//                 </div>
//                 <div className="space-y-2">
//                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Kategoria</label>
//                   <select value={formData.category} onChange={(e)=>setFormData({...formData, category: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600 font-bold transition-all">
...
//                   <input required type="number" step="0.01" value={formData.price} onChange={(e)=>setFormData({...formData, price: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600 font-bold transition-all" />
//                 </div>
//                 <div className="space-y-2">
//                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Sasia</label>
//                   <input required type="number" value={formData.stock_quantity} onChange={(e)=>setFormData({...formData, stock_quantity: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600 font-bold transition-all" />
//                 </div>
//               </div>
...
//                 type="submit" 
//                 className="w-full bg-red-600 text-white font-black py-6 rounded-[1.5rem] hover:bg-slate-900 transition-all uppercase tracking-[0.2em] shadow-2xl shadow-red-600/20 flex items-center justify-center gap-3 mt-4"
//               >
//                 {(createMutation.isPending || updateMutation.isPending) ? (
//                   <Loader2 className="animate-spin" />
//                 ) : (
//                   <><CheckCircle size={20}/> {editingProduct ? 'Përditëso Stokun' : 'Ruaj në Stok'}</>
//                 )}
//               </button>
//             </form>
//           </div>
    ...
    <div className="flex h-screen bg-[#f8fafc]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden italic">
        ...
        <main className="flex-1 overflow-y-auto p-10">
          {children} {/* KËTU do të shfaqet InventoryPage i cili tani do të jetë brenda QueryProvider */}
        </main>
      </div>
    </div>
  );
